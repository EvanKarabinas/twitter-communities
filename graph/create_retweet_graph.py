...
for month in range(1, 6):
    G = nx.Graph()
    print("\n\n\n")
    starting_date = datetime.datetime(2020, month, 1)
    ending_date = datetime.datetime(2020, month+1, 1)
    print(f"{starting_date} - {ending_date}")
    if(input_graph_name == 'both'):
        cur.execute("""SELECT tweet_id,tweet2.user_id_str AS tweet_user_id, original_tweet_id,tweet1.user_id_str
                    ...
                    INNER JOIN tweet AS tweet2 ON tweet_id=tweet2.id_str LEFT JOIN hashtag ON tweet_id_str=tweet_id WHERE  
                    tweet2.created_at >= %s AND tweet2.created_at<= %s  ORDER BY tweet_id;""",
                    (starting_date, ending_date))
    else:
        cur.execute("""SELECT tweet_id,tweet2.user_id_str AS tweet_user_id, original_tweet_id,tweet1.user_id_str
                    AS original_tweet_user_id,tweet2.created_at,hashtag_text FROM retweet INNER JOIN tweet AS tweet1 ON original_tweet_id=tweet1.id_str
                    INNER JOIN tweet AS tweet2 ON tweet_id=tweet2.id_str LEFT JOIN hashtag ON tweet_id_str=tweet_id WHERE tweet1.graph_name=%s AND 
                    tweet2.created_at >= %s AND tweet2.created_at<= %s  ORDER BY tweet_id;""",
                    (input_graph_name, starting_date, ending_date))
    ...
    # break
    nx.write_gexf(
        G, f"output_graphs/{output_file}_retweet_graph_{starting_date}.gexf")
    graph_stats(G)
